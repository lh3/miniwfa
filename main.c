...
int main(int argc, char *argv[])
{
	...
	mwf_opt_init(&opt);
	while ((c = ketopt(&o, argc, argv, 1, "cKdep:autl:f:", 0)) >= 0) {
		...
		else if (o.opt == 'f') opt.max_occ = atoi(o.arg);
	...
	if (argc - o.ind < 2) {
		...
		fprintf(stderr, "  -f INT   max k-mer occurrence [%d]\n", opt.max_occ);
	...
	while (kseq_read(ks1) >= 0 && kseq_read(ks2) >= 0) {
		mwf_rst_t rst;
		if (mode == 0) mwf_wfa_exact(0, &opt, ks1->seq.l, ks1->seq.s, ks2->seq.l, ks2->seq.s, &rst);
		else if (mode == 1) mwf_wfa_chain(0, &opt, ks1->seq.l, ks1->seq.s, ks2->seq.l, ks2->seq.s, &rst);
		else if (mode == 2) mwf_wfa_auto(0, &opt, ks1->seq.l, ks1->seq.s, ks2->seq.l, ks2->seq.s, &rst);
